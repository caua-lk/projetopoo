...
while True:
    for evento in pygame.event.get():  # Captura eventos do teclado e mouse
        ...
        if evento.type == pygame.KEYDOWN: # Teclas pressionadas
            estado_jogo = telas.Alternar_telas(evento.key, estado_jogo, nave_jogada)
            if estado_jogo.valor == "inicio":
                nave_jogada, malvados = reiniciar_jogo()
        if evento.type == pygame.MOUSEBUTTONDOWN: # Botão direito
           for botao in telas.buttons.get(estado_jogo.valor, []):
                if botao.rect.collidepoint(evento.pos):
                    if botao.text == "Confirmar Escolha":
                        if telas.nave_selecionada:
    ...
    if estado_jogo.valor == "jogando":
        ...
        nave_jogada.movimentação(pressionada)  # Move a nave com base nas teclas pressionadas
        nave_jogada.atirar()  # Verifica se a nave deve atirar

        malvados.atualiza()  # Atualiza posição e estado dos inimigos

        nave_jogada.detectar_tiro(malvados.aliens, explosions)  # Verifica se a nave foi atingida
