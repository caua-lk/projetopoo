...
from power import Powerup_Damage, Powerup_Life, Powerup_Shield
...
SizeCommom = (96,96)
Death_Sound = pygame.mixer.Sound("Jogo/Sounds/Explosion-Song.wav")

Alien_simple_image = pygame.image.load("Jogo/sprites/inimigo_simple_def.png").convert_alpha()
...
Alien_slow_image = pygame.image.load("Jogo/sprites/inimigo_slow_def.png").convert_alpha()
Alien_slow_image = pygame.transform.scale(Alien_slow_image, SizeCommom)
Alien_slow_image = pygame.transform.rotate(Alien_slow_image,180)

Alien_fast_image = pygame.image.load("Jogo/sprites/inimigo_rapido.png").convert_alpha()
Alien_fast_image = pygame.transform.scale(Alien_fast_image, SizeCommom)
Alien_fast_image = pygame.transform.rotate(Alien_fast_image,180)

Alien_teleport_image = pygame.image.load("Jogo/sprites/inimigo_teleport_def.png").convert_alpha()
Alien_teleport_image = pygame.transform.scale(Alien_teleport_image, SizeCommom)
Alien_teleport_image = pygame.transform.rotate(Alien_teleport_image,180)

Ali_tel_blank_image = pygame.image.load('Jogo/sprites/inimigo_teleport_next.png').convert_alpha()
Ali_tel_blank_image = pygame.transform.scale(Ali_tel_blank_image, SizeCommom)
Ali_tel_blank_image = pygame.transform.rotate(Ali_tel_blank_image, 180)

Alien_seach_image = pygame.image.load("Jogo/sprites/inimigo_search_def.png").convert_alpha()
...
Alien_gun_image = pygame.image.load("Jogo/sprites/inimigo_gunner_def.png").convert_alpha()
...
gun_sound = pygame.mixer.Sound("Jogo/Sounds/Gun-Sound.wav")
# --------------------- Classe de Inimigos ---------------------
class Alien:
    ...
    def queda(self):
        ...
        if (self.rect.left + 5) <= 5 or (self.rect.right - 5) >= Largura:
            self.lado *= -1

    def desenho(self, tela):
        tela.blit(self.imagem, self.rect)
    def morreu(self, explosions, power_list):
        channel = pygame.mixer.find_channel()
        if channel:
            channel.play(self.Death_sound)
        explosions.append(Explosion(self.rect.center))
        if random.random() < 0.1:
            powerups = [ Powerup_Shield, Powerup_Damage, Powerup_Life ]
            luck = random.choice(powerups)
            power_up = luck(self.rect.center)
            power_list.append(power_up)
...
class alien_simple(Alien):
    def __init__(self):
        ...
        self.rect = self.imagem.get_rect(midtop=(random.randint(50, Largura - 50), 0))
class alien_fast(Alien):
    def __init__(self):

        # Fraco e frequente
        super().__init__(saude=6,points=30,velocidade=4)

        self.imagem = Alien_fast_image

        self.rect = self.imagem.get_rect(midtop=(random.randint(50, Largura - 50), 0))
    def queda(self):
        self.rect.y += self.velocidade

class alien_slow(Alien):
    def __init__(self):

        # Fraco e frequente
        super().__init__(saude=6,points=15,velocidade=1)

        self.imagem = Alien_slow_image

        self.rect = self.imagem.get_rect(midtop=(random.randint(50, Largura - 50), 0))
    def queda(self):
        self.rect.y += self.velocidade
class alien_teleport(Alien):
    def __init__(self):
        super().__init__(saude=6, points=40, velocidade=1)
        self.imagem_base = Alien_teleport_image
        # cria uma versão totalmente branca do mesmo tamanho
        self.imagem_white = Ali_tel_blank_image
        
        self.imagem = self.imagem_base
        self.count = 0
        self.flash_time = 0
        self.rect = self.imagem.get_rect(midtop=(random.randint(50, Largura - 50), 0))
    ...
    def queda(self):
        ...
        if self.count >= 160:
            ...
            self.count = 0
            self.flash_time = 15  # fica branco por 15 frames

        # alterna imagem
        if self.flash_time > 0:
            self.imagem = self.imagem_white
            self.flash_time -= 1
        else:
            self.imagem = self.imagem_base
...
class alien_seach(Alien):
    def __init__(self):
        ...
        super().__init__(saude=12,points=50,velocidade=3)
...
class Maus:
    ...
    def atualiza(self):
        ...
        valia = [
            ...
            min(0.6 + tempo_jogo * 0.0001, 0.8),  # alien_fast
            min(0.5 + tempo_jogo * 0.0001, 0.7),  # alien_slow
            min(0.4 + tempo_jogo * 0.0001, 0.6),  # alien_teleport
        ]
